...
- Arrow IPC container inspection retained internally during rule evaluation
- parsed GeoParquet metadata retained internally during rule evaluation
...
- parsed Vortex postscript and footer metadata retained internally during rule
  evaluation
...
Arrow IPC framing, GeoParquet metadata, Iceberg metadata JSON, the latest Lance
manifest, and Vortex container metadata are parsed once by their scanners and
retained outside the serialized report model. This avoids reparsing for every
rule and prevents complete format metadata from being duplicated into JSON and
SARIF reports.
